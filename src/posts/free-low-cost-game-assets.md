...
以二手 RTX 3060 12GB（约 ¥1500）为例，对比云服务器长期使用成本：

| 使用场景 | 云服务器（AutoDL 3090） | 本地 3060 | 结论 |
|---------|----------------------|----------|------|
| 每天 2 小时，持续 1 个月 | ¥2/h × 2h × 30天 = **¥120** | ¥1500（一次性） | 云服务 |
| 每天 2 小时，持续 6 个月 | ¥120 × 6 = **¥720** | ¥1500 | 云服务 |
| 每天 2 小时，持续 1 年 | ¥120 × 12 = **¥1440** | ¥1500 | 持平 |
| 每天 4 小时，持续 1 年 | ¥240 × 12 = **¥2880** | ¥1500 | **本地省 ¥1380** |
| 每天 2 小时，持续 2 年 | ¥120 × 24 = **¥2880** | ¥1500 | **本地省 ¥1380** |

**回本周期计算：**

```
RTX 3060 12GB 二手价格：¥1500
云服务器 3090 价格：约 ¥2/小时

回本时间 = 1500 ÷ 2 = 750 小时

如果每天用 2 小时：750 ÷ 2 = 375 天 ≈ 12 个月回本
如果每天用 4 小时：750 ÷ 4 = 187 天 ≈ 6 个月回本
如果每天用 8 小时：750 ÷ 8 = 94 天 ≈ 3 个月回本
```

**本地部署的额外优势：**

1. ✅ **无网络依赖** - 断网也能用，不受平台限制
2. ✅ **无时间限制** - 想跑多久跑多久，不用算钱
3. ✅ **隐私安全** - 数据完全本地，不上传云端
4. ✅ **一次配置** - 环境配好后随时可用，不用每次重新部署
5. ✅ **可折旧出售** - 用完还能二手卖出，回收部分成本

**云服务器适合的场景：**

1. 偶尔使用（每月不超过 10 小时）
2. 需要顶级算力（A100/H100）但不想投资
3. 尝试体验，还没决定是否长期使用
4. 没有条件装显卡（如笔记本用户）

**结论：如果你计划长期使用 AI 绘图，自购显卡是更划算的选择。**
...
一台电脑确实可以安装多张显卡，但在 AI 绘图场景下有一些特殊考虑：

| 配置方式 | 显存是否叠加 | 适用场景 | 复杂度 |
|---------|-------------|---------|--------|
| 单卡 | - | 大多数用户 | 简单 |
| 多卡并行（批量生成） | ❌ 不叠加 | 同时生成多张图 | 中等 |
| 多卡显存合并 | ⚠️ 有限支持 | 运行超大模型 | 复杂 |

**多显卡的实际作用：**

1. **批量生成加速** ✅
   - 多张卡可以同时生成不同的图片
   - 例如：2 张 3060 可以同时出 2 张图，效率翻倍
   - ComfyUI 和 A1111 都支持指定使用哪张 GPU

2. **显存叠加** ⚠️ 有限制
   - 默认情况下，显存**不会自动叠加**
   - 2 张 12GB 显卡 ≠ 24GB 可用显存
   - 每张卡只能使用自己的显存运行模型
   - 但可通过特殊技术实现（见下文）

3. **模型并行（显存合并）** 🔧 需要配置
   - 使用 `accelerate` 库可以将模型拆分到多卡
   - ComfyUI 支持 `--multi-gpu` 参数（实验性）
   - 需要卡之间通过 NVLink 或 PCIe 通信，有性能损耗

**硬件要求：**

| 组件 | 双卡配置要求 | 三卡/四卡配置要求 |
|------|-------------|------------------|
| **主板** | 至少 2 个 PCIe x16 插槽 | ATX/E-ATX 大板，多 PCIe 插槽 |
| **电源** | 750W+ | 1000W-1500W |
| **机箱** | 标准 ATX，注意显卡间距 | 全塔机箱，良好散热 |
| **CPU** | 需支持足够的 PCIe 通道 | Intel X 系列或 AMD 线程撕裂者 |

**常见多卡方案：**

| 方案 | 配置 | 总价参考 | 效果 |
|------|------|---------|------|
| 入门双卡 | 2× RTX 3060 12GB | ¥2500-3000 | 批量生成效率翻倍 |
| 性价比双卡 | 2× RTX 3090 24GB | ¥7000-9000 | 专业工作站级别 |
| 极致便宜 | 2× P40 24GB | ¥1600-2400 | 需要服务器主板+改造 |
| 混合配置 | RTX 4090 + RTX 3060 | - | 4090 主力，3060 辅助 |

**实用建议：**

1. **大多数人不需要多卡**
   - 单张 RTX 3090 24GB 已经能满足绝大多数需求
   - 多卡配置复杂，功耗高，性价比不一定好
...
3. **注意事项：**
   - 两张卡之间要留足够间距，否则散热会很差
   - 功耗是大问题，2× 3090 = 700W，需要 1000W 电源
   - 不同型号的卡可以混插，但不能合并显存

4. **更简单的替代方案：**
   - 如果只是显存不够，优先考虑单张大显存卡（3090/4090）
   - 云服务器按需租用，比自建多卡更灵活
...
| 配件 | 推荐配置 | 最低配置 | 预算参考 |
|------|---------|---------|---------|
| **CPU** | Intel i5-12400 / AMD R5 5600 | i3-12100 / R5 3600 | ¥700-1000 |
| **主板** | B660/B550（带 PCIe x16） | H610/A520 | ¥400-600 |
| **内存** | 32GB DDR4 3200MHz | 16GB DDR4 | ¥400-500 |
| **硬盘** | 1TB NVMe SSD | 512GB SSD | ¥400-500 |
| **电源** | 550W 80Plus 铜牌 | 500W | ¥250-350 |
| **机箱** | 标准 ATX（支持 30cm 显卡） | - | ¥150-200 |
| **显卡** | RTX 3060 12GB | - | ¥1500 |
| **合计** | - | - | **约 ¥3800-4650** |

> 💡 如果已有电脑，只需确保：电源 500W+、有空余 PCIe x16 插槽、机箱能装下显卡

**RTX 3060 12GB 购买避坑指南：**

1. **确认是 12GB 版本**
   - 3060 有 12GB 和 8GB 两个版本，**必须买 12GB**
   - 8GB 版本是后期出的阉割版，性价比很低

2. **品牌选择**
   - 一线：华硕、微星、技嘉、七彩虹（iGame 系列）
   - 二线：影驰、索泰、映众
   - 二手建议选一线品牌，做工和散热更好
...
3. **常见型号对比**
   ...
   | 型号 | 散热规格 | 长度 | 建议 |
   |------|---------|------|------|
   | 单风扇/ITX | 单风扇 | ~17cm | ❌ 散热差，不推荐 |
   | 双风扇标准 | 双风扇 | ~24cm | ✅ 够用，性价比高 |
   | 三风扇旗舰 | 三风扇 | ~30cm | ✅ 散热好，但贵且大 |

4. **二手购买检查清单**
   ```
   □ 用 GPU-Z 确认型号：GeForce RTX 3060，显存 12GB
   □ 检查显卡外观：金手指无氧化、无明显维修痕迹
   □ 风扇转动正常，无异响
   □ 运行 FurMark 烤机 10 分钟，温度 < 85°C
   □ 测试所有视频输出接口正常
   □ 核对序列号是否与包装一致（如有）
   ```
...
1. **关机断电**，拔掉电源线
2. 打开机箱侧板，找到 PCIe x16 插槽（最长的那个）
3. 拆除对应的机箱挡板
4. 将显卡对准插槽，垂直按下直到卡扣锁定
5. 插上显卡供电线（3060 需要 1 个 8pin 或 12pin）
6. 固定显卡到机箱
...
1. **下载 NVIDIA 驱动**
   - 官网：https://www.nvidia.cn/drivers/
   - 选择：GeForce RTX 3060 → Windows 10/11 64-bit
   - 推荐选择 **Studio 驱动**（比 Game Ready 更稳定）
...
3. **验证安装**
   - 右键桌面 → NVIDIA 控制面板
   - 或运行 `nvidia-smi` 命令查看显卡信息

### 第四步：安装 Stable Diffusion

**方案 A：一键安装包（推荐新手）**

1. **下载绘世启动器（SD-WebUI 整合包）**
   - 地址：https://www.bilibili.com/video/BV1iM4y1y7oA（B站秋葉整合包）
   - 或搜索"秋葉 Stable Diffusion 整合包"
   - 优点：开箱即用，自带汉化和常用插件
...
2. **解压到纯英文路径**
   ...
   ✅ D:\SD-WebUI\
   ❌ D:\软件\AI绘画\  （中文路径会报错）
   ❌ C:\Users\张三\Desktop\  （用户名中文会报错）
   ```

3. **运行启动器**
   - 双击 `启动器.exe` 或 `webui-user.bat`
   - 首次启动会自动下载模型，需要等待
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| **CUDA out of memory** | 显存不足 | 降低分辨率、启用 `--medvram` |
| **黑屏/无输出** | 显卡没插好/没驱动 | 重新插拔显卡、安装驱动 |
| **启动报错 Python** | Python 版本不对 | 使用 Python 3.10.x |
| **下载模型失败** | 网络问题 | 使用代理或手动下载 |
| **中文路径报错** | 路径含中文 | 移动到纯英文路径 |
| **torch 安装失败** | pip 源问题 | 换国内源或手动装 |
| **生成速度很慢** | 没启用优化 | 安装 xformers |
| **模型加载失败** | 模型文件损坏 | 重新下载，检查 MD5 |

**常见坑总结：**

1. ⚠️ **路径必须是纯英文**，包括用户名
2. ⚠️ **Python 版本用 3.10**，不要用 3.11/3.12
3. ⚠️ **首次启动很慢**，需要下载依赖，耐心等待
4. ⚠️ **模型文件很大**，基础模型 2-7GB，准备好硬盘空间
5. ⚠️ **显卡驱动要新**，建议 535 以上版本
6. ⚠️ **杀毒软件可能拦截**，添加白名单
...
- `--no-half-vae`：防止生成黑图
- `--medvram`：如果 12GB 还不够用时添加

**生成参数建议：**

| 模型 | 推荐分辨率 | 批次大小 | 说明 |
|------|-----------|---------|------|
| SD 1.5 | 512×512 ~ 768×768 | 4 | 很流畅 |
| SDXL | 1024×1024 | 1 | 流畅 |
| SDXL + Refiner | 1024×1024 | 1 | 可用，稍慢 |
