...
属性定义了元素的行为、关联和功能。我们将在本系列的[属性](/web/html/attributes)部分介绍更多属性。目前只需注意，有些属性是全局性的，这意味着它们可以出现在任何元素的开始标记中。有些仅适用于部分元素，而非全部，而有些则只适用于元素，只与单个元素相关。
...
此图片有三个属性：`src`、`alt` 和 `ismap`。`src` 属性用于提供 SVG 图片资源的位置。`alt` 属性提供描述图片内容的替代文本。`ismap` 属性为布尔值，不需要值。这只是解释什么是属性。我们将在[图片](/web/html/images)部分详细介绍这些属性。
...
由于标签具有语义含义，因此您选择的元素以及您使用的标签应当适合您显示的内容。元素的语义（即 `role`）对于辅助技术和（在某些情况下）搜索引擎非常重要。[](/web/html/semantic-html)HTML 应该用于设计内容的结构，而不是定义内容的外观。外观是 CSS 的一大重点。虽然许多会改变内容外观的元素（例如 `<h1>`、`<strong>` 和 `<em>`）都具有语义含义，但外观可以而且通常会随作者样式而改变。
