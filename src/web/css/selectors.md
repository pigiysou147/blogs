...
如需了解详情，请参阅[伪类模块](/web/css/pseudo-classes)。
...
如需了解详情，请参阅[关于伪元素的模块](/web/css/pseudo-elements)。

## 复杂的选择器

您已经了解了丰富的选择器，但有时您需要使用 CSS 进行更*精细的控制*。这正是复杂的选择器的用武之地。

此时值得注意的是，虽然以下选择器为我们提供了更大的功能，但我们只能**向下级联**，选择子元素。我们无法向上定位并选择父元素。 我们将在[后续课程](/web/css/the-cascade)中介绍什么是级联及其工作原理。
...
这不会将红色应用于所有链接，也不会将红色应用于 `` 元素中的 `.my-class`。**if**如需了解详情，请参阅[特异性模块](/web/css/specificity)。
